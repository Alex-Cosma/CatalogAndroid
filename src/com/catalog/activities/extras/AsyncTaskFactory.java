...
import com.catalog.activities.StatisticsActivity;
import com.catalog.activities.TimetableActivity;
import com.catalog.activities.charts.BarChart;
import com.catalog.activities.charts.LineChart;
...
public class AsyncTaskFactory {
	...
	public AsyncTask<Object, Void, Integer> getTask(Object ctx,
		...
		/**
		 * DetailedClassStudentsDetailsFragment
		 */
		else if (className
				.equals(Constants.DetailedClassStudentsDetailsFragment)) {
			if (methodName.equals(Constants.Method_GetMarks)) {
				return new GetStudentMarksTask(
						(DetailedClassStudentsDetailsFragment) ctx);
			} else if (methodName.equals(Constants.Method_GetAttendance)) {
				return new GetStudentAttendenceTask(
						(DetailedClassStudentsDetailsFragment) ctx);
			} else
				return null;
		}
		 ...
		 * StatisticsActivity
		 */
		else if (className.equals(Constants.StatisticsActivity)) {
			return new GetStatisticsTask((StatisticsActivity) ctx);
		} else
			return null;
	}
	...
	/**
	 * GetSubjects task for DetailedClassStudentsDetailsFragment
	 * 
	 * @author Alex
	 * 
	 */
	private class GetStudentMarksTask extends AsyncTask<Object, Void, Integer> {
		private WeakReference<DetailedClassStudentsDetailsFragment> mActivityRef;
		private TextView marksText;
		private TextView avarageText;
		private StudentMarksVM studentMarksVM;
		private int position;

		public GetStudentMarksTask(DetailedClassStudentsDetailsFragment activity) {
			mActivityRef = new WeakReference<DetailedClassStudentsDetailsFragment>(
					activity);
		}
		...
		protected void onPreExecute() {
			...
			DetailedClassStudentsDetailsFragment activity = mActivityRef.get();

			if (activity == null) {
				return;
			}

			((DetailedClassActivity) activity.getActivity()).showLoading();
		...
		protected Integer doInBackground(Object... params) {
			...
			marksText = (TextView) params[2];
			avarageText = (TextView) params[3];
			position = (Integer) params[4];

			final DetailedClassStudentsDetailsFragment activity = mActivityRef
					.get();
			...
			studentMarksVM = api.getMarksForStudent(studentId, subjectId);

			activity.avarageBuffer[position] = String.valueOf(studentMarksVM
					.getMean());

			ArrayList<StudentMark> marks = studentMarksVM.getStudentMarkList();
			activity.marksBuffer[position] = "";
			if (marks.size() > 0)
				for (StudentMark element : marks) {
					// TODO: temporary, Alex..
					if (element.getDate() != null)
						activity.marksBuffer[position] += element.getMark()
		...
		protected void onPostExecute(Integer result) {
			...
			if (activity == null)
				return;
			try {
				avarageText.setText(activity.avarageBuffer[position]);
				marksText.setText(activity.marksBuffer[position]);
				activity.gradesAndAttendances.get(position).getMarks()
						.addAll(studentMarksVM.getStudentMarkList());
				activity.m_adapter.notifyDataSetChanged();
				((DetailedClassActivity) activity.getActivity()).hideLoading();
			} catch (Exception e) {
				return;
			}
			return;
	 ...
	 * 
	 */
	private class GetStudentAttendenceTask extends
			AsyncTask<Object, Void, Integer> {
		private WeakReference<DetailedClassStudentsDetailsFragment> mActivityRef;
		private TextView attendanceText;
		private AttendanceVM attendance;
		private int position;

		public GetStudentAttendenceTask(
				DetailedClassStudentsDetailsFragment activity) {
			mActivityRef = new WeakReference<DetailedClassStudentsDetailsFragment>(
					activity);
		}
		...
		protected Integer doInBackground(Object... params) {
			int studentId = (Integer) params[0];
			int subjectId = (Integer) params[1];
			attendanceText = (TextView) params[2];
			...
			attendance = api.getAttendanceForStudent(studentId, subjectId);

			if (attendance == null)
				return FAIL;

			activity.attendanceBuffer[position] = "";
			ArrayList<Attendance> attendances = attendance.getAttendanceList();
			if (attendances == null) {
				((DetailedClassActivity) activity.getActivity()).hideLoading();
				return FAIL;
				...
				for (Attendance element : attendances) {
					// TODO: Temporary if...
					if (element.getDate() != null)
						activity.attendanceBuffer[position] += dateFormat
								.format(element.getDate()) + "\n";
					else
						activity.attendanceBuffer[position] += "N/A\n";
				}
			return SUCCESS;
		}
		...
		@Override
		protected void onPostExecute(Integer result) {
			if (mActivityRef == null || result == FAIL) {
				return;
			}

			DetailedClassStudentsDetailsFragment activity = mActivityRef.get();

			if (activity == null || attendance == null) {
				return;
			}
			try {
				attendanceText.setText(activity.attendanceBuffer[position]);
				activity.gradesAndAttendances.get(position).getAttendaces()
						.addAll(attendance.getAttendanceList());
				activity.m_adapter.notifyDataSetChanged();
				((DetailedClassActivity) activity.getActivity()).hideLoading();
			} catch (Exception e) {
				return;
	...
	private class AddMarkTask extends AsyncTask<Object, Void, Integer> {
		...
		protected Integer doInBackground(Object... params) {
			...
			if (stfc.getId() != null && student.getId() != null)
	...
	private class GetStatisticsTask extends AsyncTask<Object, Void, Integer> {

		private WeakReference<StatisticsActivity> mActivityRef;
		private List<ChartData> chartData;
		private String typeOfChart;

		public GetStatisticsTask(StatisticsActivity activity) {
			mActivityRef = new WeakReference<StatisticsActivity>(activity);
		}

		@Override
		protected Integer doInBackground(Object... params) {
			typeOfChart = (String) params[0];

			chartData = api.getStatisticsDataForChart(typeOfChart);
		...
		@Override
		protected void onPostExecute(Integer result) {
			StatisticsActivity activity = mActivityRef.get();
			if (activity == null) {
				return;
			}
			activity.hideLoading();
			...
			if (result == SUCCESS) {
				...
				Intent i = (new LineChart(activity, chartData).execute());
				if (i != null)
